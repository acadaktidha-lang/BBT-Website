...
function LayoutContent() {
  ...
  // ✅ JSON-based specialization fetch (no supabase)
  const fetchSpecializations = async () => {
    try {
      const res = await fetch('/data/specializations.json', { cache: 'no-store' });
      if (!res.ok) throw new Error(`Failed to fetch specializations.json (${res.status})`);
      ...
      console.error('Failed to load specializations from JSON:', err);
      setSpecializations([]); // safe fallback
    }
  };
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="flex items-center justify-between h-24">
            ...
            <div className="hidden md:flex items-center space-x-6 lg:space-x-8">
              <Link
                to="/"
                className={`text-sm font-semibold transition-all duration-300 hover:text-orange-300 relative px-2 py-1 ${isHomePage ? 'text-orange-300' : 'text-white/90'
                  } after:content-[''] after:absolute after:w-full after:h-0.5 after:bg-orange-300 after:bottom-0 after:left-0 ${isHomePage ? 'after:scale-x-100' : 'after:scale-x-0'
              ...
              <Link
                to="/about"
                className={`text-sm font-semibold transition-all duration-300 hover:text-orange-300 relative px-2 py-1 ${location.pathname === '/about' ? 'text-orange-300' : 'text-white/90'
                  } after:content-[''] after:absolute after:w-full after:h-0.5 after:bg-orange-300 after:bottom-0 after:left-0 ${location.pathname === '/about' ? 'after:scale-x-100' : 'after:scale-x-0'
                  } after:transition-transform after:duration-300 hover:after:scale-x-100`}
              >
                About
              </Link>
              ...
              <Link
                to="/co-working"
                className={`text-sm font-semibold transition-all duration-300 hover:text-orange-300 relative px-2 py-1 ${location.pathname === '/co-working' ? 'text-orange-300' : 'text-white/90'
                  } after:content-[''] after:absolute after:w-full after:h-0.5 after:bg-orange-300 after:bottom-0 after:left-0 ${location.pathname === '/co-working' ? 'after:scale-x-100' : 'after:scale-x-0'
                  } after:transition-transform after:duration-300 hover:after:scale-x-100`}
              >
                Co-Working
              </Link>
              ...
              <Link
                to="/careers"
                className={`text-sm font-semibold transition-all duration-300 hover:text-orange-300 relative px-2 py-1 ${location.pathname === '/careers' ? 'text-orange-300' : 'text-white/90'
                  } after:content-[''] after:absolute after:w-full after:h-0.5 after:bg-orange-300 after:bottom-0 after:left-0 ${location.pathname === '/careers' ? 'after:scale-x-100' : 'after:scale-x-0'
                  } after:transition-transform after:duration-300 hover:after:scale-x-100`}
              >
                Careers
              </Link>
          ...
          {isMobileMenuOpen && (
            <div className="md:hidden backdrop-blur-xl border-t border-black/50 relative overflow-hidden">
              ...
              <div className="px-4 py-6 space-y-4 relative z-10">
                <Link
                  to="/"
                  className={`block text-sm font-semibold transition-all duration-300 hover:text-orange-300 py-3 px-4 rounded-lg ${isHomePage ? 'text-orange-300 bg-orange-300/10' : 'text-white/90 hover:bg-white/10'
                    }`}
                ...
                <Link
                  to="/about"
                  className={`block text-sm font-semibold transition-all duration-300 hover:text-orange-300 py-3 px-4 rounded-lg ${location.pathname === '/about' ? 'text-orange-300 bg-orange-300/10' : 'text-white/90 hover:bg-white/10'
                    }`}
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  About
                ...
                <div className="space-y-2">
                  ...
                  <div className="pl-4 space-y-2">
                    {specializations.length === 0 ? (
                      <div className="text-sm text-white/60 py-2 px-4">
                        No specializations available
                      </div>
                    ) : (
                      specializations.map((spec) => (
                ...
                <Link
                  to="/co-working"
                  className={`block text-sm font-semibold transition-all duration-300 hover:text-orange-300 py-3 px-4 rounded-lg ${location.pathname === '/co-working' ? 'text-orange-300 bg-orange-300/10' : 'text-white/90 hover:bg-white/10'
                    }`}
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Co-Working
                ...
                <Link
                  to="/careers"
                  className={`block text-sm font-semibold transition-all duration-300 hover:text-orange-300 py-3 px-4 rounded-lg ${location.pathname === '/careers' ? 'text-orange-300 bg-orange-300/10' : 'text-white/90 hover:bg-white/10'
                    }`}
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Careers
...
<footer className="text-white relative overflow-hidden mt-32">
  ...
  <div className="absolute inset-0 overflow-hidden">
    <div
      className="absolute top-20 left-10 w-96 h-96 bg-blue-500/25 rounded-full blur-3xl animate-pulse"
      style={{ animationDelay: '0s', animationDuration: '8s' }}
    ></div>
    <div
      className="absolute top-40 right-20 w-80 h-80 bg-blue-600/20 rounded-full blur-3xl animate-pulse"
      style={{ animationDelay: '3s', animationDuration: '10s' }}
    ></div>
    <div
      className="absolute bottom-40 left-1/3 w-72 h-72 bg-blue-400/30 rounded-full blur-3xl animate-pulse"
      style={{ animationDelay: '1s', animationDuration: '6s' }}
    ></div>
    <div
      className="absolute top-1/2 right-1/4 w-64 h-64 bg-blue-500/20 rounded-full blur-3xl animate-pulse"
  ...
  {/* ✅ MAIN FOOTER CONTENT (THIS WAS MISSING) */}
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-1 gap-4">
            {[
              ...
              { name: 'Specializations', path: '/' }, // no dedicated page route in your router
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4 text-white/85">
            <div className="flex items-center gap-3">
              ...
              <span>+92 326 0188811</span>
            </div>
            <div className="flex items-center gap-3">
              <Mail className="h-5 w-5 text-accent" />
              <span>hiring@bigbinarytech.com</span>
            </div>
            <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-4 pt-2">
            <button className="p-3 rounded-xl bg-white/10 hover:bg-white/15 transition">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
          <div className="text-white/90 text-center md:text-left">
              <p className="font-semibold text-lg">
                &copy; 2026 Big Binary Tech International. All rights reserved.
              </p>
          ...
          <div className="flex space-x-8 text-sm">
            <button className="text-white/70 hover:text-accent transition-colors font-medium hover:underline">
              Privacy Policy
            </button>
            <button className="text-white/70 hover:text-accent transition-colors font-medium hover:underline">
              Terms of Service
